...
>
	<div use:motion bind:this={containerRef} class={cn('relative h-screen', className)}>
		...
		>
			<div
				use:motion
				class="bg-grid-white/[0.2] absolute flex h-full w-full items-center justify-center bg-black [mask-image:url(/mask.svg)] [mask-size:40px] [mask-repeat:no-repeat] text-6xl text-white"
			>
